...
> 想看完整文档（进阶参数、可组合的 building blocks 等），见 [English README](README.md)。

## 适合谁、能做什么

`video-prep` 是给**对着镜头说话的口播视频**用的——小红书 / 抖音 / B站 / YouTube 的口播、
vlog、教程、网课、播客录音、产品讲解……只要你是"手机/相机怼脸录一堆素材"，
就是它的菜。如果你的日常是*"录了一堆 take，然后花一小时一段段掐头去尾"*，
这一小时它替你干。

它把每条这类视频都要做的重复清理自动化：

- ✂️ **剪掉废话间隙**——静音停顿、长停顿自动删掉
- 💬 **自动字幕**，任意语言（默认普通话），和画面对齐
- ⏩ **统一收紧节奏**，轻微加速（保留音调，不会变"花栗鼠音"）
- 🔗 **多段素材一键拼接**成完整成片，按顺序排好、字幕烧进画面
- 🗣️ **删口头禅**（然后、就是、um…）——工具先建议，删哪些你来定

它**不做**的：创作性剪辑。转场、配乐、b-roll、调色、最终编排仍在你的剪辑软件里
（剪映 / Premiere / DaVinci…）。它把干净、带字幕、可直接排版的素材交给你，
让你跳过枯燥的部分，直接进入好玩的环节。

## 环境要求（装一次即可）

不用手动装 Python 包——`uv` 和插件会替你搞定。机器上真正需要先准备的只有
`uv` 和 `ffmpeg`（想把字幕烧进预览画面，再加一个 `ffmpeg-full`）。
如果让你的编程 Agent 帮你装（见[安装](#安装)），这些它也能帮你一起装好。

工具用 `uv` 来管理 Python 和依赖。机器上需要：
...
- **我用 Claude Code** → 把这两行粘进去：
  ```
  /plugin marketplace add lucyfengcl-art/video-prep
  /plugin install video-prep
  ```
  然后直接说*"清理我的素材"*。
- **我用 Codex（或其他命令行 Agent）** → 对它说：*"帮我从
  github.com/lucyfengcl-art/video-prep 安装 video-prep 技能，并清理我的素材"*，
  它会自己跑安装（`uv tool install …`）。
- **两个都还没有** → 先装 [Claude Code](https://claude.com/claude-code)
  （或 [Codex](https://developers.openai.com/codex/cli/)），再照上面对应那行做。
  它就是一个跑在终端里的编程助手；装好之后，上面两行就够了。

> Claude Code 小提示：Agent 不能替你输入 `/plugin` 命令，它会把命令丢回来让你粘贴
> （10 秒的事）。Codex 则不需要这一步。

下面是手动 / 进阶路线——想自己用命令行的话看这里。
